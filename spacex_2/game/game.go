...
func tempoBomba() {
	...
	ticker3 := time.NewTicker(600 * time.Millisecond)
	for {
		select {
		case <-ticker3.C:
			if QtdTiros > 0 {
...
func controleNave() {
	...
	if Nave.Pjx <= -145 && Nave.DirxJ == -1 {
		Nave.DirxJ = 0
	}
	if Nave.Pjy >= 165 && Nave.DiryJ == 1 {
		Nave.DiryJ = 0
	}
	if Nave.Pjy <= -130 && Nave.DiryJ == -1 {
		Nave.DiryJ = 0
	}
...
func instaBarra(screen *ebiten.Image) {
	...
	if QtdTiros >= 0 && !Nave.FireAllowd && !Nave.IsFire && !sub {
		fmt.Println("sub")
		QtdTiros = 0
		//Nave.FireAllowd = true
		sub = true
...
func updateBomba() {
	...
	i := Bomba.Sprites.Num
	if status.Pontos >= 0 && status.Pontos <= 10 {

		Bomba.Sprites.Sprites[i].Exists = true

		//Bomba.Sprites.Sprites[i].X = rand.Float64() * float64(window.ScreenWidth*0.5))
		Bomba.Sprites.Sprites[i].X = (rand.Float64()*float64(window.ScreenWidth*0.9) + (Nave.Pjx + 150)) / 2
		Bomba.Sprites.Sprites[i].Y = 0
		Bomba.Sprites.Sprites[i].Vel = 2
		fmt.Println(Bomba.Sprites.Sprites[i].X)
	}
	...
	//fmt.Println(math.Abs(Nave.Pjx + 150))
